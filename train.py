...
def train(x_train,y_train,learning_rate = 0.05, epochs = 10):
    prediction_visual = visual_CNN(x_input)
    prediction_audio = audio_CNN(x_input)
    cost_v = tf.losses.mean_squared_error(y_input,prediction_visual)
    cost_a = tf.losses.mean_squared_error(y_input,prediction_audio)
    cost = cost_a + cost_v
