...
if __name__=='__main__':
    ...
    a = deframe(vad,win_len,hop_len)
    begin_index = []
    plot_these(a[0:len(s)],s)
    print(s.size)
    out = a[0:len(s)]
    word_index = []
    plot_this(out)
    ...
    num_ele = len(word_index)
    if out[0]==1:
        begin_index=word_index[1::2]
        end_index = word_index[::2]
        begin_index = np.insert(begin_index,0,0,axis=0)
    elif out[0] ==0:
        begin_index = word_index[::2]
        end_index = word_index[1::2]

begin_index = np.asarray(begin_index)
end_index = np.asarray(end_index)

word_one = s[begin_index[1]:end_index[1]]

print(begin_index,end_index)
wav.write('x.wav',fs,word_one)